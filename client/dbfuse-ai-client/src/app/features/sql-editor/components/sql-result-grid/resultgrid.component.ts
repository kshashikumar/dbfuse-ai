                ...
                next: (data) => {
                    if (data) {
                        if (Array.isArray(data.queries)) {
                            ...
                            // Single-query (SQL or NoSQL)
                            const normalized = this.normalizeSingleResult(data);

                            this.queryResults = [];
                            this.activeQueryIndex = 0;
                            this.resultsChanged.emit([]); // parent hides tabs

                            this.setData(normalized.rows);
                            this.totalRows = normalized.totalRows || 0;
                            this.totalPages =
                                normalized.pagination?.totalPages || Math.ceil(this.totalRows / this.pageSize) || 1;
                            if (normalized.pagination?.page) {
                                this.currentPage = normalized.pagination.page;
                            }
                            if (normalized.pagination?.pageSize) {
                                this.pageSize = normalized.pagination.pageSize;
                            }
                        }
                    } else {
                        this.setData([]);
