...
import { ConnectionConfig, DatabaseType, newTabData, openAIEvent } from '@core/utils/storage/storage.types';
...
export class LayoutHorizontalComponent implements OnInit {
    ...
    setActiveView(view: 'explorer' | 'chat'): void {
        this.activeView = view;
        this._cdr.markForCheck();
    }

    get effectiveDbType(): DatabaseType {
        const stored = getSafeSessionStorage().getItem('dbType') as DatabaseType | null;
        return this.currentDbType || stored || 'mysql2';
    }
    ...
    handleChatSendToEditor(payload: { sql: string; dbName: string }): void {
        this.pendingQuery = { ...payload, id: Date.now() };
    ...
    private async connectToServer(connection: ConnectionConfig): Promise<void> {
        try {
            ...
            this.currentDbType = connection.dbType as DatabaseType;
            this._cdr.markForCheck();
        } catch (err) {
