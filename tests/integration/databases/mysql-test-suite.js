...
const { DEFAULT_CONFIG, DB_TYPES, DB_DEFAULTS, HEADERS } = require("../../../src/core/constants");
...
class MySQLTester {
  ...
  // ---- helpers to tolerate different API response shapes ----
  extractRows(payload) {
    // New shape: payload.queries[0].rows (and possibly multiple queries)
  ...
  extractPagination(payload) {
    // New shape: pagination may be attached per-query
    if (Array.isArray(payload?.queries)) {
  ...
  extractColumnCount(payload) {
    ...
    if (Array.isArray(rows)) return rows.length; // e.g., DESCRIBE returns one row per column
    ...
    if (Array.isArray(payload?.fields)) return payload.fields.length; // some drivers return fields
    return null;
  }
  // ---- end helpers ----
  ...
  async runTests() {
    ...
    this.log(
      `MySQL Config: ${this.config.username}@${this.config.host}:${this.config.port}`,
      "info",
    );
    this.log("═".repeat(60), "info");
    ...
    await this.test("Simple SELECT Query", async () => {
      ...
      // New response shape debug (optional)
      // console.log(result.data.queries?.[0]?.rows);
      const rows = this.extractRows(result.data); // now detects queries[0].rows
    ...
    await this.test("Query Pagination", async () => {
      ...
      const pagination = this.extractPagination(result.data); // checks queries[0].pagination
