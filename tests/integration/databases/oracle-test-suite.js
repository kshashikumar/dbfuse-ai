...
class OracleTester {
  ...
  // ---- helpers to tolerate different API response shapes (including queries[0].rows) ----
  extractRows(payload) {
    // Preferred new shape: payload.queries[0].rows (support multi-query)
  ...
  extractColumnCount(payload) {
    ...
    if (Array.isArray(rows)) return rows.length; // DESCRIBE-like output usually 1 row per column
  ...
  async runTests() {
    ...
    this.log(
      `Oracle Config: ${this.config.username}@${this.config.host}:${this.config.port}/${this.config.database}`,
      "info",
    );
    this.log("═".repeat(60), "info");
    ...
    await this.test("DUAL Table Query", async () => {
      ...
      this.log(`DUAL query returned ${rows.length} row`, "info");
    });
    ...
    await this.test("Oracle System Views", async () => {
      ...
      this.log(`System views query returned ${rows.length} tablespaces`, "info");
    });
